...
- reusable requirement-intake, workflow-manager, workflow-incidents, and workflow-console Skills;
...
Workspace deployment consumes the current clean checkout and remains digest-approved and stale-state checked. Runtime UUID selection is machine-level configuration stored at `~/.multica/workflows/<workflow-id>/runtime-maps/<workspace-id>.json`. Workflow and Workspace namespacing prevents unrelated workflows or Workspaces from reusing Runtime identities, while multiple checkouts of the same workflow share the intended local selection.

Repository-specific execution artifacts remain under `<repo>/.multica/`: deployment Plans, Apply journals, immutable deployment evidence, formal release Plans, and local worktrees. Diagnostic snapshots remain under `<repo>/exports/`. This keeps portable source separate from both machine configuration and checkout-bound evidence.
...
Protocol v4 does not execute or infer older workflow protocols, metadata contracts, release evidence, or maintenance records. The reconciler recognizes existing ownership markers only for one-way cleanup: it removes retired roster bindings and automations, reassigns preserved historical Projects, archives retired Agents, and deletes retired Skills. All active objects are then rebuilt or adopted from current v4 desired state.
