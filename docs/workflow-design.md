...
Non-isolated acquisition validates a complete authority-domain inventory. For each terminal Requirement the domain contains exactly the Implementation authority and final integration-validation mirror; historical Task lease fields are residue, not additional authorities. The explicit Delivery Policy recovery command can normalize only terminal role-state or released-with-owner pairs, processes multiple groups in stable order, and requires a fresh full reread after every single metadata write. It does not assume platform CAS. Checkpoints use one compact scalar record but still preflight UTF-8 record bytes and the endpoint's total metadata byte limit. Acquisition remains blocked until every group is canonical and no current claim exists.
...
Protocol v4 does not generally execute or infer older workflow protocols, release evidence, maintenance records, or development-delivery defaults. Its only operational metadata compatibility path is the explicit fail-closed terminal lease normalization contract above; it never rewrites historical Plans or delivery evidence. Unconfigured application repositories immediately use the current delivery defaults. The reconciler recognizes existing ownership markers only for one-way cleanup: it removes retired roster bindings and automations, reassigns preserved historical Projects, archives retired Agents, and deletes retired Skills. All active objects are then rebuilt or adopted from current v4 desired state.
