...
9. Passed verification restores only source Issues whose blocking metadata is still owned by this Incident. It does not traverse descendants. Repeated close is idempotent, and a later report with the same dedupe key creates a recurrence.
