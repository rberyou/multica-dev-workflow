...
6. Before recovery, run the zero-write `integration-review --action recover`, `lease-transition`, and `incident-transition` preflights as applicable. A legacy integration validation keeps its Incident blocker while a wrong held lease is first released and then acquired by the Integrator, after which owner/reviewer records are repaired and a new UUID-routed handoff and independent Review epoch are approved. Do not infer or apply any status write that a validator did not return.
7. Record the direct `close --result passed` command or host wrapper `close-incident --result passed` with concise redacted verification evidence, a full 40-character source commit, and a 64-character deployment Plan digest. `close` preflights every blocked source before its first write and resumes exact checkpoints after partial failure. A successor blocker is installed while status stays blocked; a normal restore writes active status last. A failed check or drift keeps the Incident in `in_fix`.

While an Incident owns a source, only its transition may write status, waiting_on, blocked_reason, Incident owner, or previous status. Lease recovery is restricted to lease keys. If an incomplete Incident record remains after owner clearing, other validators still treat that namespace as reserved. A completed record is idempotent and may later be superseded by a new Incident without deleting history.
