...
The release contains one archive for each Skill in `workflow.json`, one deployable workflow archive, and `checksums.txt`. The workflow archive is an explicit allowlist containing `VERSION`, `requirements.txt`, the workflow manifest and schema, deployment profiles, runtime instructions, `workflow.py`, `workflow_lib.py`, `package_skills.py`, active Skills, and the generated `release-manifest.json`. It excludes GitHub metadata, tests, top-level development documentation, and release-only tooling. The internal manifest records a SHA-256 hash for every published file and a digest-bound portable source identity. Skill packaging uses tracked and non-ignored source files, so local ignored caches or credentials cannot enter an asset. `publish` refuses a daemon-managed Agent identity, invokes `gh release create` using the authenticated human host, and automatically marks SemVer prerelease versions as prereleases. Fetch the created tag before local snapshot verification.
