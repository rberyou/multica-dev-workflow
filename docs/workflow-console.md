...
```text
python skills/multica-workflow-console/scripts/workflow_console.py status --repo <workflow-source> --workspace <workspace>
```
...
It validates the Git checkout or extracted Release Bundle with `workflow.py doctor` and then runs `workflow.py drift` against the current workspace. Exit status is zero only when both checks pass and no drift exists.
