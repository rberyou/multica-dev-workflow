...
- Bind RC4 provenance to approved Phase 1 Maintenance Case evidence, ordinary development integration-validation provenance, the Release Request, workflow run, Environment approval actor, Publisher installation and CI, while preserving legacy maintenance-review evidence verification.
...
- Correct RC4 current-main finalization evidence to accept the approved Phase 1 Maintenance Case plus ordinary development integration-validation record, require protected tag annotations to bind case and implementation provenance hashes, and document that Dispatcher does not inspect Publisher App identity while malformed or incorrect present `bypass_actors` fail closed.
