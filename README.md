...
Non-isolated lease acquisition uses a complete, read-only terminal Requirement authority-domain preflight. Proven retired historical authority/mirror tuples, including asymmetric `released`/`held_by_integrator`, may be classified `retired_terminal_compatible` without rewriting metadata. Active or unknown tuples, incomplete endpoint discovery, stale Plan/Review/delivery or Git evidence, and current claims still fail closed. New lease writes remain canonical `held|released` only.
