...
审查绑定 workflow_stage、当前 plan_revision、delivery_policy_digest、base_commit_sha 和 reviewed_commit_sha。新 Plan 先用 Delivery Policy Skill 的 `verify-approved` 验证冻结摘要，并要求返回的 workspace/PR 选择等于当前 Issue；已有 schema-v1/v2 Plan 才使用其旧完整 snapshot 验证。验证通过后比较两个不可变 SHA 的真实 diff，不覆盖原 delivery_policy_digest。
