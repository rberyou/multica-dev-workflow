...
根据已批准任务 DAG 创建任务子 issue。每个任务 stage 等于 DAG 拓扑层；第一可执行 stage 为 todo，后续为 backlog，最后 stage 必须包含集成验证。写入 owner/reviewer/integrator、plan_revision、dependency_contract、base/target/head branch。
