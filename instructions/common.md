...
branch_only 和 lightweight 在 acquire 前必须运行 Delivery Policy 的 `lease-transition --action acquire-preflight`，fresh-read 全部终态 Requirement、唯一 Implementation authority、最终 integration-validation mirror 及 current claims。该判定只读：完全证明已退休的历史 tuple 可标记为 `retired_terminal_compatible`，但不得写 metadata、返回 normalization 或 transition record。普通 Task 的旧 lease 字段不是 authority。任何 held、未知状态、活跃 holder、端点缺失、证据或 Git 漂移、inventory 不完整或 current claim 都必须阻塞。新 lease 只写 canonical `held|released`，释放时清空两个 owner。
