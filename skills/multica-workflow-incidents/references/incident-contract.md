...
`link-fix` validates and recovers the external binding or accepts one existing protocol-v4 ordinary Requirement for legacy compatibility. Neither mode permits replacement or conflicting reverse bindings. Standard passed closure requires the fix Requirement to be `done` and non-empty redacted evidence. External closure requires typed immutable fix and deployment verification references; `git_commit` references are full commits. Standard legacy closure continues to require a full source commit and deployment Plan digest.

The only cancelled-fix exception is explicit `--closure-mode independent_remediation`. It is valid only when the unchanged linked object is a legacy ordinary protocol-v4 Requirement whose status is exactly `cancelled`, the Incident remains `in_fix`, and the result is `passed`. The caller supplies a typed immutable fix reference, a typed immutable deployment-verification reference, and non-empty redacted evidence. The script records `incident_closure_mode=independent_remediation` plus those typed references while preserving `fix_requirement_id`; it does not replace or supersede the cancelled fix. Mutable aliases such as `latest`, `current`, `head`, `main`, or `master` are rejected. Active, done, external, unlinked, or non-`in_fix` cases fail closed.

Restoration clears only blocking metadata still owned by the closing Incident. It does not traverse or automatically change descendants. Failed verification leaves the Incident in `in_fix` with mode-appropriate `waiting_on`.
