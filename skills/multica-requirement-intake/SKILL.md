...
- Do not freeze or propagate a delivery-policy digest during Requirement intake. If a user explicitly requests a repository precheck, label it diagnostic and non-authoritative; it is transient input only. The Planner's independently reviewed compact Plan contract (`plan_revision`, `policy_digest`, `target_branch`) is the first frozen policy contract.
