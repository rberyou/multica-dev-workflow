...
2. Read the top-level Issue, active descendants, recent comments, metadata, assignees, task runs, branch/commit/PR bindings, CI, tests, review outcomes, and `waiting_on`/`blocked_reason` changes.
...
Post only after explicit user authorization, completed Implementation, successful integration validation, current acceptance evidence, and a valid final Requirement revision:
...
On `done`, report the delivered PR/commit, validation evidence, acceptance results, and residual risks. On `cancelled`, report the reason and replacement link when present. On abnormal failure or actionable stall, report the owner, evidence, waiting condition, and required recovery action while continuing to follow when supported.
