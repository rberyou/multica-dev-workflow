...
The Integrator owns scheduling and maintains `workspace_lease_scope`, `workspace_lease_owner_issue_id`, `workspace_lease_owner_agent_id`, and `workspace_lease_state`. New writes use only `held` with both owners or `released` with both owners cleared. A non-isolated lease is handed from Developer to Code Reviewer to Integrator, never shared. Release it only after the current checkout is clean and at its recorded branch/head.

Before acquisition, use the complete authority-domain preflight in [lease-transition-contract.md](lease-transition-contract.md). It is read-only: a proven terminal historical authority/mirror pair may be classified `retired_terminal_compatible`, but no historical metadata is normalized. Task residue is not an authority. Unknown or active tuples, incomplete discovery, stale evidence, Git drift, or any current claim block acquisition.
