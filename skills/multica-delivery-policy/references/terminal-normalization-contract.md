...
`superseded-task-release` is not a general lease transition. Its separate approved authority binds the exact Workspace/root/Plan/superseded Implementation/target task/original owner tuple, policy and manifest identity, old and replacement branches, worktree path, PR number/head, blocker digest, and immutable base/reviewed/merged SHA plus Review comment, Reviewer, and merge method. The live snapshot must match every authority value.
